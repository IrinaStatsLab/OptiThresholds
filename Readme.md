...
The corresponding R package `OptiThresholdR` can also be downloaded by running the following:
```r
...
See `Simulations.ipynb`, `Real-data.ipynb`, and `Real-data-ai-readi.ipynb` for details. To reproduce the experiments on the AI-READI dataset, we need an R package `ClarkeTest`, imported by the Python library `rpy2`.
...
The figures for real data experiments are generated by `figure_creation.R`.
